...
4. खालील code paste करा:

```javascript
/**
 * WhatsApp Absence Notification Webhook
 * ज्ञानसिंधू क्लासेस - गैरहजेरी सूचना
 */

// WhatsApp Business API Configuration
const WHATSAPP_CONFIG = {
  // Meta Business से ये values लें
 ...
 * app.html से यह function call होगा
 */
function doPost(e) {
  try {
    // Parse incoming data
    ...
    if (!data.parentPhone || !data.studentName || !data.date) {
      return createResponse(false, 'Missing required fields');
    }
    
    // Send WhatsApp message
    const result = sendWhatsAppMessage(data);
    
    // Log the notification attempt
    logNotification(data, result);
...
function sendWhatsAppMessage(data) {
  try {
    ...
    const payload = {
      ...
      to: data.parentPhone, // Format: 919175432344
      type: "template",
      template: {
        ...
        components: [
          {
            type: "body",
            parameters: [
              { type: "text", text: data.parentName || "पालक" },
    ...
    if (responseCode === 200 && responseBody.messages) {
      return {
        success: true,
      ...
      return {
        success: false,
        message: responseBody.error?.message || 'Failed to send WhatsApp message',
        data: responseBody
      };
    }
...
/**
 * Log notification attempt to Google Sheet (optional)
 */
function logNotification(data, result) {
  try {
    // You can create a Google Sheet to log all notifications
    const SHEET_ID = 'YOUR_GOOGLE_SHEET_ID'; // Optional: Create a sheet to track notifications
    
    // If you want to track notifications, uncomment below:
    /*
...
function testWebhook() {
  const testData = {
    parentPhone: '919175432344', // Replace with your test number
    parentName: 'श्री रमेश पाटील',
    studentName: 'राज पाटील',
    date: '11 जून 2024'
  };
  
  const result = sendWhatsAppMessage(testData);
